...
struct reorder : public dnnl::reorder {
  // static void compute(const engine &src_engine, const tensor::desc &src_desc,
  //                     const engine &dst_engine, const tensor::desc &dst_desc,
  //                     const primitive_attr &aattr = primitive_attr()) {
  // }

  // XPZ: TODO: reorder between two devices
  static void compute(const tensor &src, tensor &dst) {
    dnnl::reorder(src, dst)
        .execute(stream::default_stream(), src, dst);
  }
};

void tensor::feed_from(const tensor& src) {
  reorder::compute(src, *this);
}
...
tensor tensor::to_public(void* array, bool scale_out) const {
  tensor dst {get_dims(), get_data_type(), array};
  reorder::compute(*this, dst);
  return dst;
}
// {
//   tensor ret;
...
struct convolution_forward : public dnnl::convolution_forward {
  ...
  static void compute(
    ...
    auto src_desc = src.get_desc_as_format_any();
    auto weights_desc = weights.get_desc_as_format_any();
    auto bias_desc = bias.get_desc_as_format_any();
    auto dst_desc = dst.get_desc_as_format_any();
    ...
    auto prim_desc =
        dnnl::convolution_forward::primitive_desc(op_desc, attr, aengine);

    auto expected_src = src;
    auto expected_src_desc = prim_desc.src_desc();
    if (expected_src_desc != src.get_desc()) {
      // if (!src.has_twin()) {
      //   src.init_twin(expected_src_desc);
      // }
      // expected_src = src.get_twin();
      expected_src = tensor(expected_src_desc);
      reorder::compute(src, expected_src);
    }

    auto expected_weights = weights;
    auto expected_weights_desc = prim_desc.weights_desc();
    if (expected_weights_desc != weights.get_desc()) {
      // if (!weights.has_twin()) {
      //   weights.init_twin(expected_weights_desc);
      // }
      // expected_weights = weights.get_twin();
      expected_weights = tensor(expected_weights_desc);
      reorder::compute(weights, expected_weights);
    }

    auto expected_dst_desc = prim_desc.dst_desc();
    if (expected_dst_desc != dst.get_desc()) {
      dst.reinit(expected_dst_desc);
    }

    auto comp = dnnl::convolution_forward(prim_desc);
...
struct convolution_transpose_backward_data : public dnnl::deconvolution_backward_data {
...
struct convolution_transpose_backward_weights : public dnnl::deconvolution_backward_weights {
  template<bool with_gradb = true>
  static void compute(const tensor& src, const tensor& grady, const tdims_t& gradw_dims,
...
struct batch_normalization_forward_inference : public dnnl::batch_normalization_forward {
...
struct batch_normalization_forward_training : public dnnl::batch_normalization_forward {
